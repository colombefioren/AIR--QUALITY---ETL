...
def test_clean_air_quality_data_full():
    df = pd.DataFrame({
        "pm2_5": [10.0, np.nan, 20.0],
        "pm10": [np.nan, 15.0, 25.0],
        "city_name": ["A", "B", None],
    })
    result = DataFrameCleaner.clean_air_quality_data(df)
    assert result["pm2_5"].isnull().sum() == 0
    assert result["pm10"].isnull().sum() == 0
    assert result["city_name"].iloc[2] == "unknown"


def test_clean_aqi_data_dedup_by_city_hour():
    df = pd.DataFrame({
        "city_name": ["Paris", "Paris", "London"],
        "datetime": ["2026-01-01 10:00", "2026-01-01 10:00", "2026-01-01 10:00"],
        "aqi": [2, 3, 1],
        "pm2_5": [10.0, 12.0, 8.0],
    })
    result = DataFrameCleaner.clean_aqi_data(df)
    assert len(result) == 2
    assert "London" in result["city_name"].values


def test_clean_aqi_data_fills_pollutant_nulls():
    df = pd.DataFrame({
        "city_name": ["Paris"],
    ...
    result = DataFrameCleaner.clean_aqi_data(df)
    assert result["pm2_5"].iloc[0] == 0.0
    assert result["no2"].iloc[0] == 0.0


def test_clean_aqi_data_empty():
    df = pd.DataFrame()
    result = DataFrameCleaner.clean_aqi_data(df)
    assert result.empty
