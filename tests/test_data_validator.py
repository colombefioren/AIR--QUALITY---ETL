...
def test_validate_all_within_range(caplog):
    ...
    df = pd.DataFrame({"pm2_5": [10, 20, 30], "pm10": [40, 50, 60], "aqi": [1, 2, 3]})
...
def test_validate_out_of_range(caplog):
    ...
    df = pd.DataFrame({"pm2_5": [10, 999, 30]})
    DataValidator.validate(df, "Test")
    assert "has 1 values outside" in caplog.text


def test_validate_aqi_out_of_range(caplog):
    caplog.set_level(logging.WARNING)
    df = pd.DataFrame({"aqi": [1, 3, 0, 6]})
    DataValidator.validate(df, "Test")
    assert "has 2 values outside" in caplog.text
