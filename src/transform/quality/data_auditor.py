...
class DataAuditor:
    ...
    def audit_dataframe(self, df, df_name="DataFrame"):
        ...
        aqi_insights = self._audit_aqi_insights(df)
        if aqi_insights:
            audit_results["aqi_insights"] = aqi_insights
    ...
    def _audit_nulls(self, df):
        ...
        null_percentages = (null_counts / len(df)) * 100 if len(df) > 0 else null_counts * 0
    ...
    def _audit_aqi_insights(self, df):
        insights = {}

        if "aqi" in df.columns:
            aqi_series = df["aqi"].dropna()
            if not aqi_series.empty:
                dist = aqi_series.value_counts().sort_index()
                insights["aqi_distribution"] = dist.to_dict()
                logger.info("AQI distribution (1=Good → 5=Very Poor):")
                for level, count in dist.items():
                    logger.info(f"  AQI {int(level)}: {count} records")
                if aqi_series.isin([1, 2, 3, 4, 5]).all():
                    logger.info("All AQI values within valid 1-5 range")
                else:
                    logger.warning("Some AQI values outside valid 1-5 range")

        pollutant_cols = [c for c in self.AQI_POLLUTANT_COLS if c in df.columns]
        if pollutant_cols:
            ...
            logger.info(f"Found {len(pollutant_cols)} AQI pollutant columns: {pollutant_cols}")
