...
def build_dim_date(
    ...
    dt_series = pd.to_datetime(source_df["datetime"], format="ISO8601", utc=True).dt.tz_convert(timezone(timedelta(hours=3))).dt.floor("h").drop_duplicates().sort_values().reset_index(drop=True)
